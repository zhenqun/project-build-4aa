package com.ido85.platform.thymeleaf.cache;
 ...
 * @since 2.0.0
 */
public final class HierarchicalCache<K, V> implements ICache<K, V> {
    ...
    public HierarchicalCache(final String name, final boolean useSoftReferences,
    ...
    /**
     * 这个就是从Request中获取X-Area参数的方法
     *
     * @return
     */
    private String getXAreaFromHttpRequest() {
    ...
    public void put(final K key, final V value) {
        ...
        String area = getXAreaFromHttpRequest();
        TemplateCacheKey newCacheKey = null;
        if (!StringUtils.isEmptyOrWhitespace(area)) {
            TemplateCacheKey cacheKey = (TemplateCacheKey) key;
            newCacheKey = new TemplateCacheKey(
                    cacheKey.getOwnerTemplate(),
                    area + ":" + cacheKey.getTemplate(),
                    cacheKey.getTemplateSelectors(),
        ...
        final int newSize = this.dataContainer.put(newCacheKey == null ? key : (K) newCacheKey, entry);
    ...
    public V get(final K key) {
        ...
        return get(newCacheKey == null ? key : (K) newCacheKey, this.entryValidityChecker);
    }
